...
/**
 * FILE RECOVERY ENGINE — Livello professionale con ottimizzazioni performance
 *
 * OTTIMIZZAZIONI v3.0:
 * ✓ Batching intelligente: file inviati in gruppi di 100 invece che uno alla volta
 * ✓ Throttling eventi: max 1 aggiornamento progress ogni 150ms
 * ✓ Yield ultra-frequente: ogni 30 iterazioni invece di 150
 * ✓ Buffer management: controllo memoria e flush automatico
 * ✓ Deduplicazione ottimizzata con Set
 * ✓ Stream processing: UI sempre responsiva anche con 100k+ file
 *
...
const PERF_CONFIG = {
  BATCH_SIZE:           100,   // File da inviare per batch
  BATCH_FLUSH_MS:       150,   // Flush automatico del batch ogni N ms
  YIELD_EVERY:          30,    // Yield event loop ogni N iterazioni
  PROGRESS_THROTTLE_MS: 150,   // Min tempo tra aggiornamenti progress
  MAX_BUFFER_SIZE:      50000, // Max file in memoria prima di forzare flush
};
...
const SKIP_DIRS = new Set([
  ...
  'WindowsApps','servicing','assembly','Prefetch','drivers','inf','Installer',
]);
...
// ─── BATCH MANAGER — gestisce l'invio in batch dei file ──────────────────────
class BatchManager {
  constructor(emit, batchSize = PERF_CONFIG.BATCH_SIZE) {
    this.emit = emit;
    ...
    // Auto-flush ogni BATCH_FLUSH_MS
    this.startAutoFlush();
  }
  ...
  add(file) {
    ...
    // Flush se il buffer è pieno o se è passato troppo tempo
    if (this.buffer.length >= this.batchSize || 
        this.buffer.length >= PERF_CONFIG.MAX_BUFFER_SIZE) {
      this.flush();
    }
  }
  ...
  flush() {
    ...
    // Invia il batch completo in un singolo evento
    this.emit({ 
      type: 'files_batch', 
      files: this.buffer.splice(0), 
      total: this.total 
    });
...
// ─── PROGRESS THROTTLER — limita frequenza aggiornamenti progress ────────────
class ProgressThrottler {
  constructor(emit, throttleMs = PERF_CONFIG.PROGRESS_THROTTLE_MS) {
    this.emit = emit;
  ...
  update(phase, pct, msg, total) {
    const now = Date.now();
    const data = { type: 'progress', phase, pct: Math.min(100, Math.round(pct)), msg, total };
    
    // Se è passato abbastanza tempo, invia subito
    if (now - this.lastEmit >= this.throttleMs) {
      ...
      // Altrimenti salva come pending e schedula invio
      this.pending = data;
      if (!this.timer) {
...
async function runScan(opts, emit, isStopped) {
  // Inizializza batch manager e progress throttler
  const batchMgr = new BatchManager(emit);
  const progressThrottler = new ProgressThrottler(emit);
  ...
  try {
    ...
    batchMgr.flush(); // Flush esplicito dopo ogni fase
    progress('recycle', 100, `Cestino: ${batchMgr.total - r0} file intatti recuperabili`);
    if (isStopped()) {
      batchMgr.destroy();
    ...
    if (IS_WIN && opts.isAdmin && opts.mode !== 'quick' && !isUSB && isNTFS) {
      ...
      batchMgr.flush();
      progress('vss', 100, `Shadow Copy: ${batchMgr.total - r1} file trovati`);
      if (isStopped()) {
        batchMgr.destroy();
        progressThrottler.destroy();
        return emit({ type: 'done', total: batchMgr.total });
    ...
    if (opts.mode !== 'quick') {
      ...
      batchMgr.flush();
      progress('user', 100, `Cartelle utente: ${batchMgr.total} file totali`);
      if (isStopped()) {
        batchMgr.destroy();
        progressThrottler.destroy();
        return emit({ type: 'done', total: batchMgr.total });
    ...
    progress('done', 100, `Completato — ${batchMgr.total} file trovati`);
    emit({ type: 'done', total: batchMgr.total });
    ...
    for (const letter of letters) {
      ...
      for (const sidEntry of listDir(rbRoot)) {
        ...
        for (const entry of listDir(sidDir)) {
          ...
          // Yield più frequente per mantenere UI responsiva
...
async function scanVSS(drive, add, isStopped, onP) {
  ...
  // Elenca shadow copies tramite vssadmin
...
async function scanUserFolders(add, isStopped, onP) {
  ...
  for (let i = 0; i < folders.length; i++) {
    ...
    // min 512 byte per cartelle utente
...
async function scanFilesystem(drive, add, isStopped, onP) {
  ...
  for (const root of roots) {
    await walkDir(root, add, isStopped, 'Filesystem', 12, () => {
      n++;
      if (n % 200 === 0) onP(Math.min(95, n / 300), n.toLocaleString() + ' file analizzati...');
...
async function walkDir(dir, add, isStopped, source, maxDepth, onFile, minSize = 1024, depth = 0) {
  ...
  for (const e of entries) {
    ...
    // Yield MOLTO più frequente per mantenere UI ultra-responsiva
    if (++iterCount % PERF_CONFIG.YIELD_EVERY === 0) {
      await yield_();
    }
    ...
    if (e.isDirectory()) {
      ...
      // In filesystem scan salta file senza tipo riconoscibile
